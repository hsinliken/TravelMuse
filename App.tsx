...
import { Plus, FolderOpen, BookMarked, Globe, Cloud, LayoutTemplate as TemplateIcon, Type as TypeIcon, Loader2 } from 'lucide-react';

const App: React.FC = () => {
  const [projects, setProjects] = useState<Project[]>([]);
  const [templates] = useState<TravelTemplate[]>([
  ...
  useEffect(() => {
    try {
      const savedProjects = localStorage.getItem('travelmuse_projects');
      if (savedProjects) {
      ...
      // Don't crash the app, just clear invalid data
      localStorage.removeItem('travelmuse_projects');
      localStorage.removeItem('travelmuse_fonts');
    }
  }, []);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-stone-50">
        ...
        <button onClick={() => window.location.reload()} className="bg-stone-900 text-white px-4 py-2 rounded-lg">重新整理</button>
      </div>
    );
  }
          ...
          <div className="space-y-12 animate-in fade-in duration-700">
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white border border-stone-200 p-6 rounded-2xl hover:border-stone-400 transition-colors cursor-pointer group shadow-sm">
                ...
                <div 
                  ...
                  className="bg-white border border-stone-200 p-6 rounded-2xl hover:border-stone-400 transition-colors cursor-pointer group shadow-sm"
                >
                  <div className="w-10 h-10 bg-stone-50 rounded-lg flex items-center justify-center mb-4 text-stone-600 group-hover:bg-stone-900 group-hover:text-white transition-all">
                    <TypeIcon size={20} />
                  </div>
                  <h3 className="font-bold text-lg">字體管理</h3>
                  <p className="text-sm text-stone-500 mt-1">{customFonts.length} 個自訂字體</p>
                ...
                <div className="bg-white border border-stone-200 p-6 rounded-2xl hover:border-stone-400 transition-colors cursor-pointer group shadow-sm">
                  ...
                  <p className="text-sm text-stone-500 mt-1">同步連線中</p>
                </div>
                <div className="bg-white border border-stone-200 p-6 rounded-2xl hover:border-stone-400 transition-colors cursor-pointer group shadow-sm">
                  <div className="w-10 h-10 bg-stone-50 rounded-lg flex items-center justify-center mb-4 text-stone-600 group-hover:bg-stone-900 group-hover:text-white transition-all">
                    <Globe size={20} />
                  ...
                  <p className="text-sm text-stone-500 mt-1">管理線上部署</p>
                </div>
              </div>
            </section>
                  ...
                  {projects.map(project => (
                    ...
                    >
                      <div className="h-40 bg-stone-100 relative overflow-hidden">
                        <img src={`https://picsum.photos/seed/${project.id}/400/200`} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" loading="lazy" />
